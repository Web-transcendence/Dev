...
import { fetchId, fetchNotifyUser } from './utils.js'

export const Friend_db = new Database('/app/db/friend.db')  // Importation correcte de sqlite
...
export function getFriendList(id: number): FriendList {
	const accepted = Friend_db.prepare(`SELECT userA_id
                                        FROM FriendList
                                        WHERE userB_id = ?
                                          AND status = 'accepted'
                                        UNION
                                        ...
                                        FROM FriendList
                                        WHERE userA_id = ?
                                          AND status = 'accepted'`).all(id, id) as {
		userA_id?: number,
		userB_id?: number
	}[]
	const pending = Friend_db.prepare(`SELECT userB_id
                                       FROM FriendList
                                       WHERE userA_id = ?
                                         AND status = 'pending'`).all(id) as { userB_id: number }[]
	const invited = Friend_db.prepare(`SELECT userA_id
                                       FROM FriendList
                                       WHERE userB_id = ?
                                         AND status = 'pending'`).all(id) as { userA_id: number }[]
...
export async function addFriend(id: number, nickName: string) {
	...
	const checkStatus = Friend_db.prepare('SELECT status FROM FriendList WHERE userA_id = ? AND userB_id = ?').get(friendId, id) as {
		status: string
	}
	if (checkStatus?.status == 'accepted')
		...
		Friend_db.prepare(`UPDATE FriendList
                           SET status = 'accepted'
                           WHERE userA_id = ?
                             AND userB_id = ?`).run(friendId, id)
		await fetchNotifyUser([friendId], 'newFriend', { id: id })
		return `Friend invitation accepted`
	}

	const res = Friend_db.prepare(`INSERT
    OR IGNORE INTO FriendList (userA_id, userB_id, status) VALUES (?, ?, ?)`).run(id, friendId, 'pending')
	if (res.changes === 0)
