...
export class User {
	...
	static async login(nickName: string, password: string): Promise<string> {
		...
		const data = Client_db.prepare('SELECT activated2fa FROM Client WHERE id = ?').get(id) as {
	...
	static getIdbyNickName(nickName: string): number {
		const userData = Client_db.prepare('SELECT id FROM Client WHERE nickName = ?').get(nickName) as {
			id: number
		} | undefined
		if (!userData)
	...
	async isPasswordValid(password: string): Promise<boolean> {
		const userData = Client_db.prepare('SELECT password FROM Client WHERE id = ?').get(this.id) as {
			password: string
		} | undefined
		if (!userData)
	...
	async generateSecretKey(): Promise<string> {
		const data = Client_db.prepare('SELECT secret_key FROM Client WHERE id = ?').get(this.id) as {
			secret_key: string
		} | undefined
		if (!data)
			throw new DataBaseError(`User with ID ${this.id} not found`, 'internal error system', 500)
	...
	verify(token: string): string {
		const secret = Client_db.prepare('SELECT secret_key FROM Client WHERE id = ?').get(this.id) as {
			secret_key: string
		} | undefined
		if (!secret)
		...
		Client_db.prepare(`UPDATE Client
                           SET activated2fa = ?
                           WHERE id = ?`).run(1, this.id)
	...
	updatePictureProfile(pictureURL: string) {
		...
		const change = Client_db.prepare(`UPDATE Client
                                          SET pictureProfile = ?
                                          WHERE id = ?`).run(pictureURL, this.id)
	...
	getProfile(): { id: number, nickName: string, email: string, avatar: string } {
		const userData = Client_db.prepare('SELECT nickName, email, pictureProfile FROM Client WHERE id = ?').get(this.id) as {
			nickName: string,
			email: string,
	...
	publicData(): { id: number, nickName: string, avatar: string, online: boolean } {
		const data = Client_db.prepare('SELECT nickName, pictureProfile FROM Client WHERE id = ?').get(this.id) as {
			nickName: string,
			pictureProfile: string
		} | undefined
		if (!data)
