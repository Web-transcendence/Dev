...
/*   Updated: 2025/03/26 15:36:48 by thibaud          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	DeepQAgent::train( void ) {
	...
	for (int i = 0; i < this->_maxEpTraining; i++) {
		...
		for (int a = 0; a < this->_maxActions; a++) {
			...
			auto	pred_Q = this->_QNet->feedForward(*input);
		...
		Math::displayProgress(i,this->_maxEpTraining);
	}
	std::cout << std::endl;
}
...
bool	DeepQAgent::realisable( void ) {
	...
	auto	output = this->_QNet->feedForward(*testInput);
	double	res = 0.0;
...
int	DeepQAgent::policy(t_mode const mode) {
